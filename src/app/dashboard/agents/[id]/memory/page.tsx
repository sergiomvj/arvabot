...
import { Brain, MessageSquare, Clock, History, User, Globe, ChevronRight } from 'lucide-react'
...
export default async function AgentMemoryPage({ params }: { params: { id: string } }) {
  ...
  const activeThreads = agent.threads.filter(t => t.status === 'active')
  const resolvedThreads = agent.threads.filter(t => t.status === 'resolved')
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Link 
          ...
          className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#4a5580] hover:text-white hover:bg-white/10 transition-all border border-white/5"
        >
        ...
        <div>
          ...
          <p className="text-[#4a5580] text-sm">Memória Cross-Canal e Continuidade (SMT v1.1)</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${(activeThreads.length / 20) * 100}%` }}
                />
              </div>
          ...
          <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-6">
            <h3 className="text-xs font-bold text-emerald-500 uppercase tracking-widest mb-2 font-mono">Próxima Promoção</h3>
            <p className="text-[11px] text-[#8892b0] leading-relaxed">
              Threads resolvidas com alta relevância são automaticamente promovidas para o <strong>SHORT_TERM (MEMORY.md)</strong> após a próxima rodada do SMT Processor.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {activeThreads.length === 0 ? (
            <div className="py-12 flex flex-col items-center justify-center text-[#4a5580] bg-[#0C0F1A] rounded-2xl border border-white/5 border-dashed">
              ...
              <p className="text-[11px] mt-1">Interações cross-canal aparecerão aqui automaticamente.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {activeThreads.map((thread) => (
                <div key={thread.id} className="bg-[#0C0F1A] border border-white/5 rounded-2xl p-6 hover:border-emerald-500/30 transition-all group">
                  ...
                  <div className="bg-[#07090F] rounded-xl p-4 border border-white/5 relative overflow-hidden">
                    ...
                    <p className="text-[11px] text-[#8892b0] leading-relaxed whitespace-pre-wrap font-mono uppercase opacity-80">
                      {thread.context}
                    </p>
                  </div>
                </div>
          ...
          {resolvedThreads.length > 0 && (
            <>
              <h2 className="text-lg font-bold text-white flex items-center gap-2 pt-4">
                <History size={20} className="text-[#4a5580]" /> Histórico Recente (Resolvidas)
              </h2>
              <div className="space-y-3 opacity-60">
                {resolvedThreads.map((thread) => (
                  <div key={thread.id} className="bg-[#0C0F1A] border border-white/5 rounded-xl p-4 flex items-center justify-between">
                    ...
                    <div className="text-[9px] text-[#4a5580] font-mono">
                      Resolvida em {new Date(thread.last_interaction).toLocaleDateString()}
                    </div>
                  </div>
                ))}
              </div>
